...
`rollup_new_FCC()`. The most efficient way to work with the new FCC data is the
...
header. The code should still work if you decide to work with a state-level data set but the
code is originally designed to work with the U.S. data set.
...
If you are done working with the SQLite database, be sure to close the connection. 
You may also want to remove the sqlite, zip, and raw csv files from your working
...
```{r}
ak_dat <- read_csv("C:/Users/kaile/Downloads/IA-Fixed-Jun2020-v2.csv")

ak_dat %>% 
  distinct(Provider_Id, FRN) %>% 
  arrange(Provider_Id) %>% 
  count(FRN) %>% 
  filter(n > 1)

ak_dat %>% filter(FRN == "0002587863") %>% arrange(Provider_Id)
```
