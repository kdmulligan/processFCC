...
```{r}
c(html_elements(fcc_page, "p")[10][1])[[1]]
html_elements(fcc_page, "p")
html_nodes(fcc_page, "p")
library(rvest)
# pg <- read_html("https://en.wikipedia.org/wiki/Main_Page")
links <- html_nodes(fcc_page, "p")

list <- xml2::xml_children(links) %>% xml2::as_list() 
xml2::xml_children(links) %>% html_nodes("href")
...
old_dat_ver %>% 
  group_by(date) %>% 
  arrange(date, desc(version)) %>% 
  mutate(most_recent = if_else(version == max(version), 1, 0)) %>% 
  left_join(links, join_by(rn)) %>% 
...
list[[25]]
data.frame(t(sapply(list, str_flatten_comma)))

as.data.frame(xml2::xml_children(links))
xml2::xml_text(xml2::xml_children(links))

bind_rows(lapply(xml2::xml_children(links), function(x) data.frame(as.list(x), stringsAsFactors=FALSE)))
